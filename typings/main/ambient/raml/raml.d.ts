...
declare module Raml08Parser {

    type GlobalSchemas = { [schemaName: string /* SchemaName */]: SchemaDefinition }[]

    type SchemaName = string

    type SecuredBy =  Array<SecuritySchemeName | PartialSecuritySchemeDescribedByObject /* | Null*/>

    type SecuritySchemeName = string

    type PartialSecuritySchemeDescribedByObject = { [securitySchemeName: string]: { [name: string]: any } }
    ...
    type TraitReference = Array<string | { [trait: string]: ParameterMap }>

    type ResourceTypeReference = string | { [resourceType: string]: ParameterMap }
    ...
    interface Api {
         ...
         * For example, let's look at the following API provider that parametrizes the base URI with customer
    ...
    interface Resource {
         ...
         *
         * relativeUri = /foo/{bar}/baz/{xxx}
         * relativeUriPathSegments = ['bar', 'xxx']
         */
        ...
        /**
         * An array of applied traits on the current resource.
         *
    ...
    interface BodyPayload {
         ...
         */
        schema?: SchemaName | SchemaDefinition
     ...
     */
    interface Responses {
        [statusCode: string]: Response // | Null
    }


    interface NamedParameterMap {
    ...
    type NamedParameter = BasicNamedParameter | NumericNamedParameter | StringNamedParameter
}


declare module "raml-parser" {
    class FileReader {
        constructor(cb: (path: string) => Promise<string>);
    }
    ...
    function loadFile(url: string, options?: LoadOptions): Q.Promise<Raml08Parser.Api>
    function load(str: string, path?: string, options?: LoadOptions): Q.Promise<Raml08Parser.Api>
}